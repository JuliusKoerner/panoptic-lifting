    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(trajectory_loader)):
            ...
            for i in range(len(outputs[0])):
                concated_outputs.append(
                    torch.cat([outputs[j][i] for j in range(len(outputs))], dim=0)
                )
            ...
            p_instances = create_instances_from_semantics(
                p_instances, p_semantics, test_set.segmentation_data.fg_classes
            )
            ...
            stack = visualize_panoptic_outputs(
                ...
                H,
                W,
                thing_classes=test_set.segmentation_data.fg_classes,
                visualize_entropy=False,
            )
            ...
            name = (
                f"{test_set.all_frame_names[test_set.val_indices[batch_idx]]}.png"
                if test_only
                else f"{batch_idx:04d}.png"
            )
            Image.fromarray(grid).save(
                output_dir / "vis_semantics_and_surrogate" / name
            )
            Image.fromarray(
                output_semantics_with_invalid.reshape(H, W)
                .cpu()
                .numpy()
                .astype(np.uint8)
            ).save(output_dir / "pred_semantics" / name)
            Image.fromarray(
                p_instances.argmax(dim=1).reshape(H, W).cpu().numpy().astype(np.uint16)
            ).save(output_dir / "pred_surrogateid" / name)


def create_instances_from_semantics(instances, semantics, thing_classes):
    stuff_mask = ~torch.isin(
        semantics.argmax(dim=1), torch.tensor(thing_classes).to(semantics.device)
    )
    padded_instances = torch.ones(
        (instances.shape[0], instances.shape[1] + 1), device=instances.device
    ) * -float("inf")
