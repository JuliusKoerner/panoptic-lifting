...
sys.path.append("/home/rozenberszki/project/panoptic-lifting")
...
class TensoRFTrainer(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        config.instance_optimization_epoch = (
            config.instance_optimization_epoch + config.late_semantic_optimization
        )
        config.segment_optimization_epoch = (
            config.segment_optimization_epoch + config.late_semantic_optimization
        )
        self.config = config
        ...
        total_classes = len(self.train_set.segmentation_data.bg_classes) + len(
            self.train_set.segmentation_data.fg_classes
        )
        output_mlp_semantics = (
            torch.nn.Identity()
            if self.config.semantic_weight_mode != "softmax"
            else torch.nn.Softmax(dim=-1)
        )
        self.model = TensorVMSplit(
            ...
            dim_feature_instance=self.config.max_instances,
            output_mlp_semantics=output_mlp_semantics,
            use_semantic_mlp=self.config.use_mlp_for_semantics,
            use_feature_reg=self.config.use_feature_regularization,
        )
        self.renderer = TensoRFRenderer(
            self.train_set.scene_bounds,
            [config.min_grid_dim, config.min_grid_dim, config.min_grid_dim],
            semantic_weight_mode=self.config.semantic_weight_mode,
            stop_semantic_grad=config.stop_semantic_grad,
        )
        semantic_weights = get_semantic_weights(
            config.reweight_fg,
            self.train_set.segmentation_data.fg_classes,
            self.train_set.segmentation_data.num_semantic_classes,
        )
        ...
        if not self.config.use_symmetric_ce:
            self.loss_semantics = torch.nn.CrossEntropyLoss(
                reduction="none", weight=semantic_weights
            )
        else:
            self.loss_semantics = SCELoss(
                self.config.ce_alpha, self.config.ce_beta, semantic_weights
            )
        ...
        self.output_dir_result_clusters = Path(
            f"runs/{self.config.experiment}/instance_clusters"
        )
    ...
    def configure_optimizers(self):
        params = self.model.get_optimizable_parameters(
            self.config.lr * 20, self.config.lr, weight_decay=self.config.weight_decay
        )
        optimizer, scheduler = get_optimizer_and_scheduler(
            params, self.config, betas=(0.9, 0.99)
        )
        param_instance = self.model.get_optimizable_instance_parameters(
            self.config.lr * 20, self.config.lr
        )
        optimizer_instance, scheduler_instance = get_optimizer_and_scheduler(
            param_instance, self.config, betas=(0.9, 0.999)
        )
    ...
    def forward(self, rays, is_train):
        ...
        for i in range(0, B, self.config.chunk):
            (
                ...
                self.model,
                rays[i : i + self.config.chunk],
                self.config.perturb,
                self.train_set.white_bg,
                is_train,
            )
    ...
    def forward_instance(self, rays, is_train):
        ...
        for i in range(0, B, self.config.chunk):
            ...
            out_feats_instance_ = self.renderer.forward_instance_feature(
                self.model, batch_rays, self.config.perturb, is_train
            )
    ...
    def forward_segments(self, rays, is_train):
        ...
        for i in range(0, B, self.config.chunk_segment):
            ...
            out_feats_segments_ = self.renderer.forward_segment_feature(
                self.model, batch_rays, self.config.perturb, is_train
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        rays, rgbs, semantics, probs, confs, masks, feats = (
            batch[0]["rays"],
            batch[0]["rgbs"],
            batch[0]["semantics"],
        ...
        if self.config.lambda_rgb > 0:
            ...
            loss_tv_semantics = self.model.tv_loss_semantics(self.tv_regularizer) * (
                1 if self.current_epoch >= self.config.late_semantic_optimization else 0
            )
            loss_tv_appearance = self.model.tv_loss_appearance(self.tv_regularizer)
            loss_tv = (
                loss_tv_density * self.config.lambda_tv_density
                + loss_tv_appearance * self.config.lambda_tv_appearance
                + loss_tv_semantics * self.config.lambda_tv_semantics
            )
            ...
            loss = self.config.lambda_rgb * (
                loss_rgb
                + loss_tv
                + loss_dist_reg * self.current_lambda_dist_reg
                + loss_feat * self.config.lambda_feat
            )
            self.log(
                "train/loss_rgb",
                loss_rgb,
            ...
            self.log(
                "train/loss_tv_regularizer",
                loss_tv_appearance + loss_tv_density,
                on_step=True,
                on_epoch=False,
                prog_bar=False,
                logger=True,
            ...
            self.log(
                "train/loss_dist_regularizer",
                loss_dist_reg + loss_tv_density,
                on_step=True,
                on_epoch=False,
                prog_bar=True,
                logger=True,
        ...
        if self.config.probabilistic_ce_mode == "TTAConf":
            loss_semantics = (
                self.loss_semantics(output_semantics, probs) * confs
            ).mean()
        elif self.config.probabilistic_ce_mode == "NoTTAConf":
            loss_semantics = (
                self.loss_semantics(output_semantics, semantics) * confs
            ).mean()
        else:
        ...
        if (
            self.config.segment_grouping_mode != "none"
            and self.current_epoch >= self.config.segment_optimization_epoch
        ):
            ...
            batch_target_mean = torch.zeros(
                self.config.batch_size_segments,
            ...
            loss_segment_clustering = (
                self.loss_semantics(semantic_features, target) * batch[2]["confidences"]
            ).mean()
            self.log(
                f"train/loss_segment",
                loss_segment_clustering,
                on_step=True,
                on_epoch=False,
                prog_bar=True,
        ...
        if self.current_epoch >= self.config.late_semantic_optimization:
            loss = (
                loss
                + self.config.lambda_semantics * loss_semantics
                + self.config.lambda_semantics
                * self.config.lambda_segment
                * loss_segment_clustering
            )
        ...
        train_cm = ConfusionMatrix(
            num_classes=self.model.num_semantic_classes, ignore_class=[0]
        )
        metric_iou = train_cm.add_batch(
            output_semantics_without_invalid.cpu().numpy(),
            semantics.cpu().numpy(),
            return_miou=True,
        )
        self.log(
        ...
        if self.current_epoch >= self.config.instance_optimization_epoch:
            ...
            loss_instance_clustering = torch.zeros(
                1, device=self.device, requires_grad=True
            )
            for img_idx in range(len(batch[1]["rays"])):
                instance_features = self.forward_instance(
                    batch[1]["rays"][img_idx], True
                )
                loss_instance_clustering_ = self.calculate_instance_clustering_loss(
                    batch[1]["instances"][img_idx],
                    instance_features,
                    batch[1]["confidences"][img_idx],
                )
                loss_instance_clustering = (
                    loss_instance_clustering + loss_instance_clustering_
                )
            self.manual_backward(loss_instance_clustering)
            opts[1].step()
            self.log(
                f"train/loss_clustering",
                loss_instance_clustering,
                on_step=True,
                on_epoch=False,
                prog_bar=True,
    ...
    def calculate_instance_clustering_loss(
        self, labels_gt, instance_features, confidences
    ):
        virtual_gt_labels = self.create_virtual_gt_with_linear_assignment(
            labels_gt, instance_features
        )
        predicted_labels = instance_features.argmax(dim=-1)
        if torch.any(
            virtual_gt_labels != predicted_labels
        ):  # should never reinforce correct labels
            return (
                self.loss_instances_cluster(instance_features, virtual_gt_labels)
                * confidences
            ).mean()
    ...
    def create_virtual_gt_with_linear_assignment(self, labels_gt, predicted_scores):
        labels = sorted(torch.unique(labels_gt).cpu().tolist())[
        ...
        for lidx, label in enumerate(labels):
            cost_matrix[lidx, :] = (
                -(
                    predicted_probabilities[labels_gt == label, :].sum(dim=0)
                    / ((labels_gt == label).sum() + 1e-4)
                )
                .cpu()
                .numpy()
            )
    ...
    def calculate_segment_clustering_loss(self, sem_features, confidences):
        target = (
            torch.mean(torch.softmax(sem_features, dim=-1), dim=0)
            .detach()
            .unsqueeze(0)
            .expand(sem_features.shape[0], -1)
        )
        if self.config.segment_grouping_mode == "argmax_noconf":
            ...
            return (
                self.loss_semantics(sem_features, target.argmax(-1)) * confidences
            ).mean()
    ...
    def validation_step(self, batch, batch_idx):
        rays, rgbs, semantics, instances, mask = (
            batch["rays"].squeeze(),
            batch["rgbs"].squeeze(),
            batch["semantics"].squeeze(),
            batch["instances"].squeeze(),
            batch["mask"].squeeze(),
        )
        rs_semantics, rs_instances = (
            batch["rs_semantics"].squeeze(),
            batch["rs_instances"].squeeze(),
        )
        probs, confs = batch["probabilities"].squeeze(), batch["confidences"].squeeze()
        output_rgb, output_semantics, output_instances, _output_depth, _, _ = self(
            rays, False
        )
        ...
        pano_pred = torch.cat(
            [
                output_semantics_without_invalid.unsqueeze(1),
                output_instances.argmax(dim=1).unsqueeze(1),
            ],
            dim=1,
        )
        ...
        metric_pq, metric_sq, metric_rq = panoptic_quality(
            pano_pred,
            pano_target,
            self.train_set.things_filtered,
            self.train_set.stuff_filtered,
        ...
        val_rs_cm = ConfusionMatrix(
            num_classes=self.model.num_semantic_classes,
        ...
        return {
            "loss_rgb": loss.item(),
            "loss_sem": loss_semantics.item(),
            "psnr": metric_psnr.item(),
            "iou": metric_iou.item(),
            "pq": metric_pq.item(),
            "sq": metric_sq.item(),
            "rq": metric_rq.item(),
            "rs_iou": metric_rs_iou.item(),
            "rs_pq": metric_rs_pq.item(),
            "rs_sq": metric_rs_sq.item(),
            "rs_rq": metric_rs_rq.item(),
        }
    ...
    def validation_epoch_end(self, val_step_outputs):
        ...
        table.append(
            tuple(
                [
                    np.array(
                        [val_step_outputs[i][key] for i in range(len(val_step_outputs))]
                    ).mean()
                    for key in table[0]
                ]
            )
        ...
        (self.output_dir_result_clusters / f"{self.global_step:06d}").mkdir(
            exist_ok=True
        )
        ...
        for batch_idx, batch in enumerate(self.val_dataloader()):
            if batch_idx in self.config.visualized_indices:
                rays, rgbs, semantics, instances = (
                    batch["rays"].squeeze().to(self.device),
                    batch["rgbs"].squeeze().to(self.device),
                    batch["semantics"].squeeze().to(self.device),
                    batch["instances"].squeeze().to(self.device),
                )
                rs_semantics, rs_instances = batch["rs_semantics"].squeeze().to(
                    self.device
                ), batch["rs_instances"].squeeze().to(self.device)
                mask = batch["mask"].squeeze().to(self.device)
                output_rgb, output_semantics, output_instances, output_depth, _, _ = (
                    self(rays, False)
                )
                output_rgb[torch.logical_not(mask), :] = 0
                output_semantics[torch.logical_not(mask)] = 0
                ...
                stack = visualize_panoptic_outputs(
                    ...
                    H,
                    W,
                    thing_classes=self.train_set.segmentation_data.fg_classes,
                )
                save_image(
                    stack,
                    self.output_dir_result_images
                    / f"{self.global_step:06d}_{batch_idx:04d}.jpg",
                    value_range=(0, 1),
                    nrow=5,
                    normalize=True,
                )
    ...
    def train_dataloader(self):
        ...
        loaders[1] = DataLoader(
            train_instance_set,
            self.config.batch_size_contrastive,
            shuffle=True,
            drop_last=True,
            collate_fn=train_instance_set.collate_fn,
            num_workers=0,
        )
        if self.config.segment_grouping_mode != "none":
            loaders[2] = DataLoader(
                self.train_segment_set,
                self.config.batch_size_segments,
                shuffle=False,
                drop_last=True,
                collate_fn=self.train_segment_set.collate_fn,
                num_workers=0,
            )
        return loaders
    ...
    def val_dataloader(self):
        return DataLoader(
            self.val_set, batch_size=1, shuffle=False, drop_last=False, num_workers=0
        )

    def on_train_epoch_start(self):
        self.current_lambda_dist_reg = self.config.lambda_dist_reg * (
            1 - math.exp(-0.25 * self.current_epoch)
        )
        ...
        if self.current_epoch in self.config.grid_upscale_epochs:
            num_voxel_list = (
                torch.round(
                    torch.exp(
                        torch.linspace(
                            np.log(self.config.min_grid_dim**3),
                            np.log(self.config.max_grid_dim**3),
                            len(self.config.grid_upscale_epochs) + 1,
                        )
                    )
                ).long()
            ).tolist()[1:]
            target_num_voxels = num_voxel_list[
                self.config.grid_upscale_epochs.index(self.current_epoch)
            ]
