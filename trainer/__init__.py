...
from util.misc import (
    visualize_depth,
    probability_to_normalized_entropy,
    get_boundary_mask,
)
...
def create_trainer(name, config):
    ...
    ), "only 1:1 supported"  # TODO: fix dataprocessing bug limiting this
    ...
    if config.logger == "wandb":
        ...
        logger = TensorBoardLogger(
            name="tb", save_dir=(Path("runs") / config.experiment)
        )
...
def get_optimizer_and_scheduler(params, config, betas=None):
    opt = torch.optim.Adam(
        params, lr=config.lr, weight_decay=config.weight_decay, betas=betas
    )
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        opt, milestones=config.decay_step, gamma=config.decay_gamma
    )
...
def visualize_panoptic_outputs(
    p_rgb,
    p_semantics,
    p_instances,
    p_depth,
    ...
    if visualize_entropy:
        img_sem_entropy = visualize_depth(
            probability_to_normalized_entropy(
                torch.nn.functional.softmax(p_semantics, dim=-1)
            ).reshape(H, W),
            minval=0.0,
            maxval=1.00,
    ...
    img_semantics = distinct_colors.apply_colors_fast_torch(p_semantics.cpu()).view(
        H, W, 3
    ).permute(2, 0, 1) * alpha + img * (1 - alpha)
    boundaries_img_semantics = get_boundary_mask(p_semantics.cpu().view(H, W))
    img_semantics[:, boundaries_img_semantics > 0] = 0
    colored_img_instance = distinct_colors.apply_colors_fast_torch(
        p_instances.cpu()
    ).float()
    ...
    colored_img_instance[thing_mask.cpu(), :] = p_rgb.cpu()[thing_mask.cpu(), :]
    img_instances = colored_img_instance.view(H, W, 3).permute(
        2, 0, 1
    ) * alpha + img * (1 - alpha)
    if rgb is not None and semantics is not None and instances is not None:
        img_gt = rgb.view(H, W, 3).permute(2, 0, 1).cpu()
        img_semantics_gt = distinct_colors.apply_colors_fast_torch(
            semantics.cpu()
        ).view(H, W, 3).permute(2, 0, 1) * alpha + img_gt * (1 - alpha)
        boundaries_img_semantics_gt = get_boundary_mask(semantics.cpu().view(H, W))
        img_semantics_gt[:, boundaries_img_semantics_gt > 0] = 0
        colored_img_instance_gt = distinct_colors.apply_colors_fast_torch(
            instances.cpu()
        ).float()
            ...
            [
                torch.stack(
                    [
                        ...
                        img_semantics_gt,
                        img_instances_gt,
                        torch.zeros_like(img_gt),
                        torch.zeros_like(img_gt),
                    ]
                ),
                torch.stack(
                    [img, img_semantics, img_instances, depth, img_sem_entropy]
                ),
            ],
            dim=0,
        )
